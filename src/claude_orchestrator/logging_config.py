...
def setup_logging(
    ...
    Returns:
        Configured logger
    """
    # Resolve log dir from config if not provided
    if not log_dir:
        from .config import get_config
        log_dir = str(get_config().log_dir)
...
def get_logger(name: str) -> logging.Logger:
    ...
    return logging.getLogger(f"claude_orchestrator.{name}")
